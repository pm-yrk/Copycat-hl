...
const HISTORY_BASE = 'https://raw.githubusercontent.com/pm-yrk/Copycat-hl/position-history'
...
export async function loadPositionHistory(rangeMs: number, signal: AbortSignal): Promise<PositionFrame[]> {
  const read = async (path: string) => {
    const response = await fetch(`${HISTORY_BASE}/${path}`, { signal, cache: 'no-store' })
  ...
  for (let i = 0; i < days.length; i += 4) {
    const batch = await Promise.all(days.slice(i, i + 4).map(async (day) => {
      if (day !== newest && dayCache.has(day)) return dayCache.get(day)!
      ...
      dayCache.set(day, payload.frames)
