...
function fmtTime(ms: any) { if (!ms) return 'Awaiting refresh'; const d = new Date(Number(ms)); return d.toLocaleString(undefined, { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }).replace(',', '') }
...
function maskWallet(w: string) { return w ? `Wallet ${w.slice(0, 4)}…${w.slice(-4)}` : 'Wallet' }

const palette = ['#43E8D0', '#8057FF', '#44BDEC', '#FFB020', '#25D366', '#F35EA6', '#A6E22E', '#FF5B72']
const fallbackColours: Record<string, string> = { HYPE:'#43E8D0', ETH:'#627EEA', BTC:'#F7931A', SOL:'#14F195', ZEC:'#F4B728', NEAR:'#00EC97', AAVE:'#8B7DFF', TRX:'#FF4B4B', XRP:'#4B9FFF', USDC:'#2775CA', MELANIA:'#D7A785', WLD:'#8492A6' }
...
function TokenLogo({ coin, icons }: { coin: string, icons: Record<string, string> }) {
  ...
  return <span className="token-logo exact-token-logo" style={{ ['--coin' as any]: color }}>
...
function ExposureBars({ signals, icons }: { signals: any[], icons: Record<string, string> }) {
  ...
  return <div className="exposure-list exact-exposure-list">{rows.map(r => { const l = Number(r.value_long_usd || 0), s = Number(r.value_short_usd || 0); const total = l + s || 1; return <div className="ex-row exact-ex-row" key={r.coin}><div className="ex-name"><TokenLogo coin={r.coin} icons={icons} /><b>{r.coin}</b><span>{Number(r.signal).toFixed(2)}</span></div><div className="ex-track"><div style={{ width: `${Math.max(7, ((l + s) / max) * 100)}%` }}><i style={{ width: `${(l / total) * 100}%` }} /><em style={{ width: `${(s / total) * 100}%` }} /></div></div><small>{compactMoney(l)}</small><small>{compactMoney(s)}</small></div> })}</div>
}
...
export default function Dashboard() {
  ...
  return <><Nav /><main className="dashboard-shell exact-dashboard-shell"><LineBackdrop variant="dashboard" />
    <section className="exact-dashboard-top">
      <div className="exact-dashboard-left">
        <div className="dash-copy exact-dash-copy"><p className="eyebrow live">Live smart-wallet tape</p><h1>Market intelligence.<br /><em>Follow the best.</em></h1><p>Value-weighted positioning from qualified Hyperliquid wallets.<br />Built to show what serious traders are leaning into.</p></div>
        <div className="bias-block exact-bias-block"><span>Positioning bias</span><button className={`bias-pill ${isLong ? 'long' : 'short'}`}><i />{isLong ? 'LONG' : 'SHORT'}</button></div>
      </div>
      <aside className="orders-card exact-orders-card"><h3>Most recent orders</h3>{orderRows.slice(0, 3).map((o: any, i: number) => <div className="order-line" key={i}><TokenLogo coin={o.coin} icons={icons} /><b>{o.coin}</b><span className={String(o.side).toLowerCase().includes('short') || String(o.side).toLowerCase().includes('reduce') ? 'negative' : 'positive'}>{o.side}</span><em>{o.wallet_label || maskWallet(o.wallet || '')}</em><small>{ago(o.ts_ms)}</small></div>)}<button className="small-action">View all orders →</button></aside>
      <aside className="market-card exact-market-card"><h3>Market overview</h3>{marketRows.map(([label, value, change]) => <div className="market-line" key={label}><div><small>{label}</small><b>{value}</b></div><span className={change.startsWith('-') ? 'negative' : 'positive'}>{change}</span></div>)}<div className="market-foot"><span>Source: CoinGecko</span><a>View more →</a></div></aside>
    </section>

    {err && <p className="risk-bar amber-risk">{err}</p>}

    <section className="kpi-grid exact-kpi-grid"><article><i className="round-icon users" /><small>Qualified wallets</small><b>{summary.qualified_wallets || 0}</b><span>ranked daily</span><Spark /></article><article><i className="round-icon wallet" /><small>Tracked account value</small><b>{money(summary.tracked_account_value_usd)}</b><span>latest snapshots</span><Spark /></article><article><i className="round-icon chart" /><small>Open position value</small><b>{money(summary.tracked_open_position_value_usd)}</b><span>{summary.open_positions || 0} live positions</span><Spark /></article><article><i className="round-icon bolt" /><small>Assets with signals</small><b>{summary.assets_with_signals || 0}</b><span>cross-asset breadth</span><Spark /></article><article><i className="round-icon clock" /><small>Signal refresh</small><b>{fmtTime(summary.latest_signal_ts_ms)}</b><span>UTC</span></article></section>

    <section className="chart-grid exact-chart-grid"><article className="analytics-card allocation-card"><h3>Portfolio allocation</h3><AllocationDonut targets={targets} /><p className="hint">Hover segments for details</p></article><article className="analytics-card exposure-card2"><div className="panel-title"><h3>Long vs short exposure</h3><span><i />Long <em />Short</span></div><ExposureBars signals={signals} icons={icons} /></article></section>

    <section className="table-grid exact-table-grid"><article className="table-panel"><div className="panel-title"><h3>Asset signal board</h3><span>value-weighted, not wallet-count only</span></div><div className="scroll-table"><table className="exact-table"><thead><tr><th>#</th><th>Asset</th><th>Signal</th><th>Confidence</th><th>Wallets</th><th>Value L/S</th><th>Net value</th><th>% total value</th></tr></thead><tbody>{signals.slice(0, 6).map((r, i) => <tr key={r.coin}><td>{i + 1}</td><td><span className="asset-cell"><TokenLogo coin={r.coin} icons={icons} /><b>{r.coin}</b></span></td><td className={cls(r.signal)}>{Number(r.signal).toFixed(2)}</td><td><span className={`confidence ${r.confidence?.toLowerCase?.() || ''}`}>{r.confidence}</span></td><td>{r.wallets_long} long / {r.wallets_short} short</td><td>{compactMoney(r.value_long_usd)} / {compactMoney(r.value_short_usd)}</td><td className={cls(r.net_value_usd)}>{compactMoney(r.net_value_usd)}</td><td>{pct(r.value_long_pct_total)} long / {pct(r.value_short_pct_total)} short</td></tr>)}</tbody></table></div><a className="panel-link">View full signal board →</a></article><article className="table-panel"><div className="panel-title"><h3>Recent buyer / seller pressure</h3><span>largest flow changes first</span></div><div className="scroll-table"><table className="exact-table"><thead><tr><th>Asset</th><th>Net buyers</th><th>Bullish flow</th><th>Bearish flow</th><th>Net value flow</th><th>Read</th></tr></thead><tbody>{flow.slice(0, 5).map(r => { const read = flowRead(r.net_buyer_count); return <tr key={r.coin}><td><span className="asset-cell"><TokenLogo coin={r.coin} icons={icons} /><b>{r.coin}</b></span></td><td className={cls(r.net_buyer_count)}>{r.net_buyer_count}</td><td>{compactMoney(r.bullish_flow_usd || r.bullish_value_flow_usd)}</td><td>{compactMoney(r.bearish_flow_usd || r.bearish_value_flow_usd)}</td><td className={cls(r.net_value_flow_usd)}>{compactMoney(r.net_value_flow_usd)}</td><td><span className={`read ${read.toLowerCase()}`}>{read}</span></td></tr> })}</tbody></table></div><a className="panel-link">View full pressure tape →</a></article></section>
    <p className="risk-bar green-risk dashboard-risk exact-dashboard-risk"><i>♢</i><span>Market intelligence only. Not financial advice. Crypto trading can result in loss.</span><em>● Data updates every 30s</em></p>
  </main></>
}
