...
function AssetName({ coin, details, row, compact = false }: { coin: any, details: Record<string, AssetDetail>, row?: any, compact?: boolean }) {
  ...
  const gross = Number(row?.gross_exposure_usd || meta?.gross_exposure_usd || (longUsd + shortUsd) || 0)
...
export default function Dashboard() {
  ...
  return <><Nav /><main className="cc-dashboard-shell"><LineBackdrop variant="dashboard" />
      ...
      <aside className="cc-top-rail">
        <div className={`cc-orders-card ${showAllOrders ? 'expanded' : ''}`}>
          ...
          <div className="cc-order-list">
            {visibleOrders.map((o: any) => <div className="cc-order-line" key={orderKey(o)}><TokenLogo coin={o.coin} icons={icons} /><AssetName coin={o.coin} details={assetDetails} row={o} compact /><span className={orderActionClass(o.side)}>{o.side}</span><em>{o.wallet_label || maskWallet(o.wallet)}</em><small>{ago(o.ts_ms)}</small></div>)}
          </div>
    ...
    <section className="cc-chart-grid">
      <div className="cc-card cc-allocation-card"><div className="cc-card-title-row"><h3>Portfolio allocation</h3><span>Last rebalanced: {fmtTime(latestAllocationTs(targets, signals, summary))} UTC</span></div><AllocationDonut targets={targets} signals={signals} trackedValue={Number(summary.tracked_account_value_usd || 0)} icons={icons} assetDetails={assetDetails} /></div>
      <div className="cc-card cc-exposure-card"><div className="cc-panel-title"><h3>Long vs short exposure</h3><span><i />Long <em />Short</span></div><ExposureBars signals={signals} icons={icons} assetDetails={assetDetails} /></div>
    </section>
      ...
      <div className="cc-card cc-table-card">
        ...
        <div className="cc-scroll-table cc-scroll-y">
          <table className="cc-flow-table">
            ...
            <tbody>{sortedFlow.map((r, i) => { const read = flowRead(r.net_value_flow_usd); return <tr key={`${r.coin}-${i}`}><td><span className="cc-asset-cell"><TokenLogo coin={r.coin} icons={icons} /><AssetName coin={r.coin} details={assetDetails} row={r} /></span></td><td className={cls(r.net_buyer_count)}>{r.net_buyer_count}</td><td>{money(r.bullish_flow_usd)}</td><td>{money(r.bearish_flow_usd)}</td><td className={cls(r.net_value_flow_usd)}>{money(r.net_value_flow_usd)}</td><td><span className={`cc-read ${read.toLowerCase()}`}>{read}</span></td></tr> })}</tbody>
          </table>
        </div>
      </div>
    </section>
