...
function AssetCell({ coin, icons }: { coin: any, icons: Record<string, string> }) {
  const symbol = displayToken(String(coin || '—'))
  return <span className="cc-api-asset-cell"><TokenLogo coin={symbol} icons={icons} /><b>{symbol}</b></span>
}
...
export default function ApiAccessPage() {
    ...
    async function load() {
      try {
        const [s, l, t, c, feed] = await Promise.all([
          ...
          apiGet('/api/data/v1/public/leaderboard-preview?limit=10').catch(() => ({})),
          apiGet('/api/data/v1/public/token-screener-preview?limit=10').catch(() => ({})),
  ...
  const endpoints = [
    ...
    ['Ranked wallets', `${baseUrl}/api/leaderboard-preview.json`, 'Selected cohort preview.'],
  ...
  const pipeline = [
    ...
    ['Selected cohort', `${compact(selected)} tracked wallets`],
  ...
  return <><Nav/><main className="cc-dashboard-shell cc-api-intel-page"><LineBackdrop variant="dashboard" />
    <section className="cc-api-intel-hero cc-card">
      <div className="cc-api-hero-copy">
        ...
        <p>Ranked wallet cohorts, market-level positioning, recent tracked-wallet flow and public snapshot endpoints powering Copycat.</p>
      ...
      <aside className="cc-api-status-tile">
        ...
        <b>{endpoints.length} snapshot files</b>
        <em>{freshness(updatedAt)}</em>
      </aside>
    </section>

    <section className="cc-api-purpose-row">
      <article className="cc-card"><span className="cc-api-icon cc-api-icon-source"/><small>Source layer</small><b>Hyperliquid public data</b><p>Wallet state, market context and tracked activity are collected without requiring customer exchange credentials.</p></article>
      <article className="cc-card"><span className="cc-api-icon cc-api-icon-rank"/><small>Copycat layer</small><b>Ranking + signal logic</b><p>Candidate wallets are filtered into a selected cohort, then converted into market-level positioning snapshots.</p></article>
      <article className="cc-card"><span className="cc-api-icon cc-api-icon-json"/><small>Delivery layer</small><b>Read-only JSON snapshots</b><p>Public preview files power the dashboard, status page and API preview while private API access remains a later step.</p></article>
    </section>
    ...
    <section className="cc-api-nansen-grid">
      <article className="cc-card cc-api-data-card cc-api-leaderboard-card">
        <header><div><p className="eyebrow live">Ranked wallet leaderboard</p><h2>Selected wallet cohort</h2></div><span>scanner-ranked</span></header>
      ...
      <article className="cc-card cc-api-data-card cc-api-market-card">
        <header><div><p className="eyebrow live">Token screener</p><h2>Markets the cohort is leaning into</h2></div><span>top conviction</span></header>
        <div className="cc-api-tab-row"><span>5m</span><span>1h</span><b>24h</b><span>7D</span></div>
        <div className="cc-api-table-wrap"><table><thead><tr><th>Asset</th><th>Tilt</th><th>Wallets</th><th>Exposure</th><th>Net</th></tr></thead><tbody>{tokens.slice(0, 8).map((r:any, i:number) => { const label = biasLabel(r); return <tr key={r.coin || i}><td><AssetCell coin={r.coin} icons={icons} /></td><td className={biasClass(label)}>{label}</td><td>{compact(r.wallets_long)}L / {compact(r.wallets_short)}S</td><td>{money(r.gross_value_usd || r.gross_exposure_usd)}</td><td className={biasClass(r.net_value_usd)}>{signed(r.net_value_usd)}</td></tr> })}</tbody></table></div>
      </article>
    </section>
    ...
    <section className="cc-card cc-api-data-card cc-api-activity-card">
      ...
      <div className="cc-api-table-wrap"><table><thead><tr><th>Wallet</th><th>Action</th><th>Asset</th><th>Value</th><th>Time</th></tr></thead><tbody>{orders.slice(0, 8).map((r:any, i:number) => <tr key={`${r.wallet || 'wallet'}-${r.ts_ms || i}`}><td>{rowWallet(r)}</td><td className={actionClass(r.side || r.action)}>{r.side || r.action || 'Order'}</td><td><AssetCell coin={r.coin || r.asset} icons={icons} /></td><td>{money(r.delta_value_usd || r.position_value_usd || r.value_usd || r.notional_usd)}</td><td>{freshness(r.ts_ms)}</td></tr>)}</tbody></table></div>
    </section>
