...
function signalText(row: any) {
  const n = Number(row?.signal ?? 0)
  return `${Math.round(Math.abs(n) * 100)}% ${n < 0 ? 'Short' : 'Long'}`
}
...
export default function ApiAccessPage() {
  ...
  const scanned = Number(
    (feedLive && (summary.indexed_wallets || summary.known_wallet_candidates || summary.scanner_candidate_wallets_scored))
    || (statusLive && status?.scanner_candidate_wallets_scored)
    || (coverageLive && coverage?.scanner_candidate_wallets_scored)
    || 0
  )
  const selected = Number(
    (feedLive && (summary.selected_wallet_count || summary.qualified_wallets || summary.tracked_active_wallets))
  ...
  const markets = marketLive ? (screener?.rows || screener?.data || []) : []
  ...
  const codeSample = useMemo(() => {
    ...
    return JSON.stringify({
      asset: sample.coin,
      direction: Number(sample.signal) < 0 ? 'short' : 'long',
      conviction: Math.round(Math.abs(Number(sample.signal || 0)) * 100),
  ...
  return <div className="public-redesign-root">
    ...
    <main className="public-redesign-shell public-api-page">
      ...
      <section className="public-api-browser">
        ...
        {tab === 'markets' ? <div className="public-api-table-block"><h3>Market positioning <small>{freshness(marketTs || latestTs)}</small></h3>{markets.length ? <div className="public-table-scroll"><table><thead><tr><th>Asset</th><th>Conviction</th><th>Long wallets</th><th>Short wallets</th><th>Net exposure</th></tr></thead><tbody>{markets.slice(0,12).map((r:any)=><tr key={r.coin}><td><b>{r.coin}</b></td><td className={Number(r.signal)<0?'negative':'positive'}>{signalText(r)}</td><td>{compact(r.wallets_long)}</td><td>{compact(r.wallets_short)}</td><td className={Number(r.net_value_usd)<0?'negative':'positive'}>{money(r.net_value_usd)}</td></tr>)}</tbody></table></div> : <div className="public-data-empty"><b>Waiting for a fresh market snapshot.</b><span>No bundled table is shown as live.</span></div>}</div> : null}
