...
def _safe_ddl(conn, sql: str) -> None:
    try:
        ...
        # DDL can race with live writers during deploys. Existing tables are good
        # enough for workers to continue; avoid taking the product down for an
        # index/check that can be retried on the next clean deploy.
        log.warning("owned schema DDL skipped: %s", str(exc)[:220])


def ensure_owned_tables(conn) -> None:
    global _OWNED_TABLES_READY
    if _OWNED_TABLES_READY:
        return

    # In production these tables already exist. Do not re-run CREATE INDEX on
    # every worker loop; it caused statement timeouts/deadlocks while fills were
    # being inserted.
    if (
        _table_exists(conn, 'owned_wallet_fills')
        and _table_exists(conn, 'owned_wallet_metrics')
        and _table_exists(conn, 'owned_wallet_metric_history')
    ):
        _OWNED_TABLES_READY = True
        return

    got_lock = False
    try:
        got_lock = bool(conn.execute(text('SELECT pg_try_advisory_lock(82420401)')).scalar())
    except Exception:
        got_lock = False
    if not got_lock:
        # Another process is doing first-time schema setup. Avoid deadlocking.
        log.warning('owned schema setup already in progress; skipping this pass')
        return
    ...
    try:
        ...
        ''')
        _safe_ddl(conn, 'CREATE INDEX IF NOT EXISTS idx_owned_wallet_fills_wallet_ts ON owned_wallet_fills(wallet, ts_ms DESC)')
        _safe_ddl(conn, 'CREATE INDEX IF NOT EXISTS idx_owned_wallet_fills_ts ON owned_wallet_fills(ts_ms DESC)')
        _safe_ddl(conn, '''
        ...
        ''')
        _safe_ddl(conn, 'CREATE INDEX IF NOT EXISTS idx_owned_wallet_metrics_score ON owned_wallet_metrics(qualifies, score DESC)')
        _safe_ddl(conn, 'CREATE INDEX IF NOT EXISTS idx_owned_wallet_metrics_ts ON owned_wallet_metrics(ts_ms DESC)')
        _safe_ddl(conn, '''
        ...
        ''')
        _safe_ddl(conn, 'CREATE INDEX IF NOT EXISTS idx_owned_wallet_metric_history_wallet_ts ON owned_wallet_metric_history(wallet, ts_ms DESC)')
        _safe_ddl(conn, 'CREATE INDEX IF NOT EXISTS idx_owned_wallet_metric_history_ts ON owned_wallet_metric_history(ts_ms DESC)')
        _safe_ddl(conn, 'CREATE INDEX IF NOT EXISTS idx_owned_wallet_metric_history_score ON owned_wallet_metric_history(ts_ms DESC, qualifies, score DESC)')
        _OWNED_TABLES_READY = True
    finally:
        try:
            conn.execute(text('SELECT pg_advisory_unlock(82420401)'))
        except Exception:
            pass
...
def _store_fills(conn, wallet: str, fills: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for fill in fills:
        ...
        conn.execute(text('''
            ...
            ON CONFLICT(wallet, tid) DO NOTHING
        '''), row)
    return {
