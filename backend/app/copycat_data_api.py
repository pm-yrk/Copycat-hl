...
def ensure_copycat_data_api_tables(conn) -> None:
    ...
    conn.execute(text('''
        ...
        VALUES
          ('Hyperliquid official Info API', 'official_api', 'https://api.hyperliquid.xyz/info', 'official_public_api', 'Current user state, user portfolio, recent/historical user fills by time where available, funding and ledger endpoints.', 'Not a complete already-indexed Nansen-style wallet universe; Copycat must store observations permanently from collection time.', '{"priority":1}'::jsonb),
          ('Hyperliquid official WebSocket', 'official_websocket', 'wss://api.hyperliquid.xyz/ws', 'official_public_api', 'Streaming user fills, order updates, account state and market feeds for tracked wallets.', 'Requires one subscription per tracked wallet/feed; only becomes complete for wallets once Copycat is subscribed.', '{"priority":2}'::jsonb),
          ('Hyperliquid official archive bucket', 'official_archive', 's3://hyperliquid-archive', 'official_public_archive', 'Official historical market data such as L2 snapshots and asset contexts.', 'Official docs state this archive may be delayed/missing and does not provide every historical dataset such as all user fills.', '{"priority":3}'::jsonb)
