...
from .worker import Hyperliquid, is_address, now_ms, safe_float, short_wallet
...
def ensure_copycat_data_lake_tables(conn) -> None:
    ...
    '''))
    conn.execute(text('CREATE INDEX IF NOT EXISTS idx_copycat_market_universe_updated ON copycat_market_universe(updated_at DESC)'))
    ...
    '''))
    conn.execute(text('CREATE INDEX IF NOT EXISTS idx_copycat_market_snapshots_coin_ts ON copycat_market_snapshots(coin, ts_ms DESC)'))
    conn.execute(text('CREATE INDEX IF NOT EXISTS idx_copycat_market_snapshots_ts ON copycat_market_snapshots(ts_ms DESC)'))
    ...
    '''))
    conn.execute(text('CREATE INDEX IF NOT EXISTS idx_owned_wallet_backfill_status ON owned_wallet_backfill_state(last_status, updated_at)'))
    ...
    '''))
    conn.execute(text('CREATE INDEX IF NOT EXISTS idx_copycat_data_quality_ts ON copycat_data_quality_snapshots(ts_ms DESC)'))
